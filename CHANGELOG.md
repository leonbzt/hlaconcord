...
All notable changes to hlaharm are documented here. The format follows
...
### Added
- **CLI (`hlaharm`)** — `run`, `validate`, `normalize`, and `db` subcommands
...
- **Reference-database management** (`hlaharm.db`) — on-demand fetch of an
...
- **GL String export** (`hlaharm.gl`) — standardized Genotype List strings from
  the harmonized consensus.
- **Pipeline entry point** (`hlaharm.pipeline.run`) — importable parse →
  normalize → concord orchestration behind the CLI.
- **JSON report** (`hlaharm.report.concordance_json`) — sample-grouped,
...
- The IPD-IMGT/HLA database is **fetched, not redistributed** — `run` requires a
  release installed via `hlaharm db update <version>` first.
